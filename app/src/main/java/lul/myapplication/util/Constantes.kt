package lul.myapplication.util

class Constantes {

    companion object {
        const val JAVI = "Já vi"
        const val QUEROVER = "Quero ver"
        const val SIM = "SIM"
        const val NÃO = "NÃO"
        const val INITIAL_INDEX = 0
        const val SALVAR = "SALVAR"
        const val SEARCH_HINT = "Pesquisando... segura aí!"
        const val QUERY_LENGTH = 3
        const val ADD_FILME_MENSAGEM_JAVI = "Filme adicionado na lista Ja Vi"
        const val ADD_FILME_MENSAGEM_QUEROVER = "Filme adicionado na lista Quero Ver"
        const val CANCELAR = "CANCELAR"
        const val PESQUISA_DELAY = 900L
        const val TAKE_PICTURE = 1
        const val SELECT_PICTURE = 2
        const val PROFILE_PATH = "/data/user/0/lul.myapplication/app_imageDir"
        const val REQUEST_CODE = 42
    }
}